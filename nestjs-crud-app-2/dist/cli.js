...
async function manageEntregas() {
    ...
    if (action === 'Criar Entrega') {
        ...
        await axios_1.default.post('http://localhost:3000/entregas', { pedidoId, status, dataEntrega });
        console.log('Entrega criada com sucesso!');
    }
    else if (action === 'Listar Entregas') {
        const response = await axios_1.default.get('http://localhost:3000/entregas');
        ...
        const response = await axios_1.default.get(`http://localhost:3000/entregas/${id}`);
        ...
        await axios_1.default.put(`http://localhost:3000/entregas/${id}`, { status });
        console.log('Entrega atualizada com sucesso!');
    }
    else if (action === 'Remover Entrega') {
        const { id } = await inquirer_1.default.prompt([{ type: 'number', name: 'id', message: 'ID da entrega:' }]);
        await axios_1.default.delete(`http://localhost:3000/entregas/${id}`);
...
async function managePagamentos() {
    ...
    if (action === 'Criar Pagamento') {
        ...
        await axios_1.default.post('http://localhost:3000/pagamentos', { pedidoId, valor, status });
        console.log('Pagamento criado com sucesso!');
    }
    else if (action === 'Listar Pagamentos') {
        const response = await axios_1.default.get('http://localhost:3000/pagamentos');
        ...
        const response = await axios_1.default.get(`http://localhost:3000/pagamentos/${id}`);
        ...
        await axios_1.default.put(`http://localhost:3000/pagamentos/${id}`, { status });
        console.log('Pagamento atualizado com sucesso!');
    }
    else if (action === 'Remover Pagamento') {
        const { id } = await inquirer_1.default.prompt([{ type: 'number', name: 'id', message: 'ID do pagamento:' }]);
        await axios_1.default.delete(`http://localhost:3000/pagamentos/${id}`);
...
async function manageEstabelecimentos() {
    ...
    if (action === 'Criar Estabelecimento') {
        ...
        await axios_1.default.post('http://localhost:3000/estabelecimentos', { nome, endereco, telefone });
        console.log('Estabelecimento criado com sucesso!');
    }
    else if (action === 'Listar Estabelecimentos') {
        const response = await axios_1.default.get('http://localhost:3000/estabelecimentos');
        ...
        const response = await axios_1.default.get(`http://localhost:3000/estabelecimentos/${id}`);
        ...
        await axios_1.default.put(`http://localhost:3000/estabelecimentos/${id}`, { nome });
        console.log('Estabelecimento atualizado com sucesso!');
    }
    else if (action === 'Remover Estabelecimento') {
        const { id } = await inquirer_1.default.prompt([{ type: 'number', name: 'id', message: 'ID do estabelecimento:' }]);
        await axios_1.default.delete(`http://localhost:3000/estabelecimentos/${id}`);
...
async function manageItensPedido() {
    ...
    if (action === 'Criar Item do Pedido') {
        const { produtoId, quantidade, pedidoId } = await inquirer_1.default.prompt([
            { type: 'number', name: 'produtoId', message: 'ID do produto:' },
            { type: 'number', name: 'quantidade', message: 'Quantidade:' },
            { type: 'number', name: 'pedidoId', message: 'ID do pedido:' },
        ]);
        await axios_1.default.post('http://localhost:3000/itens-pedido', { produtoId, quantidade, pedidoId });
        console.log('Item do pedido criado com sucesso!');
    }
    else if (action === 'Listar Itens do Pedido') {
        const response = await axios_1.default.get('http://localhost:3000/itens-pedido');
        console.log('Itens do Pedido:', response.data);
    }
    else if (action === 'Buscar Item do Pedido') {
        const { id } = await inquirer_1.default.prompt([{ type: 'number', name: 'id', message: 'ID do item do pedido:' }]);
        const response = await axios_1.default.get(`http://localhost:3000/itens-pedido/${id}`);
        console.log('Item do Pedido:', response.data);
    }
    else if (action === 'Atualizar Item do Pedido') {
        const { id, quantidade } = await inquirer_1.default.prompt([
            { type: 'number', name: 'id', message: 'ID do item do pedido:' },
            { type: 'number', name: 'quantidade', message: 'Nova quantidade:' },
        ]);
        await axios_1.default.put(`http://localhost:3000/itens-pedido/${id}`, { quantidade });
        console.log('Item do pedido atualizado com sucesso!');
    }
    else if (action === 'Remover Item do Pedido') {
        const { id } = await inquirer_1.default.prompt([{ type: 'number', name: 'id', message: 'ID do item do pedido:' }]);
        await axios_1.default.delete(`http://localhost:3000/itens-pedido/${id}`);
